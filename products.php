...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '' || strlen($name) > 160 || !in_array($category, $allowedCategories, true) || ($sku !== '' && (strlen($sku) > 60 || !preg_match('/^[A-Z0-9-]+$/', $sku))) || !in_array($stockType, $allowedStockTypes, true) || !in_array($unit, $allowedUnits, true) || $reorderLevel === false || $reorderLevel < 0 || !is_finite((float) $reorderLevel)) {
        $error = 'Enter a valid product name and choose valid category, stock type, unit, SKU, and reorder values.';
    } else {
        try {
            $sku = $sku ?: generateSku($database, $category);
            $statement = $database->prepare('INSERT INTO products (name, sku, category, stock_type, unit, reorder_level) VALUES (?, ?, ?, ?, ?, ?)');
            $statement->execute([$name, $sku, $category, $stockType, $unit, (float) $reorderLevel]);
            $message = 'Product added to the catalog.';
        } catch (PDOException $exception) {
            $error = $exception->getCode() === '23000' ? 'That SKU is already registered.' : 'The product could not be saved.';
        }
    }
}
